...
설치를 마쳐도 **자동매매 기능은 현재 동작하지 않습니다.** 대시보드 조회 기능
(계좌 / 코인 시세 / UI 렌더링)은 정상입니다.

| # | 위치 | 증상 |
|---|---|---|
| 1 | `trading/views.py:36` | 모듈 전역에 `trader = None` 선언이 없어, 자동매매 시작 버튼을 누르면 `NameError: name 'trader' is not defined` 로 500 발생 |
| 2 | `trading/auto_trade.py:58` | `self.get_rsi()` 메서드가 구현되어 있지 않음. 1번을 고쳐도 `AttributeError` 발생 |
| 3 | `trading/templates/main.html:181-183` | 폴링 주기가 `200`ms — 초당 15회 요청이 업비트로 나가 Rate Limit(429)에 걸림. 주석의 의도대로 `1000`ms 가 맞아 보임 |
| 4 | `trading/utils.py:71` | 시장가 매도 시 `volume` 을 채우지 않음. 업비트 시장가 매도는 `volume` 이 필수라 주문이 거부됨 |
