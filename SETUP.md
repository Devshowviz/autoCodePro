...
헤더, 포지션 관리, 매도 전략, 일일 손실 한도, 웹 엔드포인트에 대한 **86개**
테스트가 있습니다.
...
- **일일 누적 손익은 메모리에만 있습니다.** 보유 종목은 DB(`TradeRecord`)에서
  복원되지만, 로스컷으로 정지된 뒤 서버를 재시작하면 누적 손익이 0으로
  초기화되어 그날 안에 다시 매매가 시작됩니다.
- 워커를 여러 개 띄우면 각 워커가 독립적으로 매매하게 되어 위험합니다.
  단일 워커로만 운영하세요.
- `FailedMarket` 에 쌓인 종목은 **영구히 매수 대상에서 제외**됩니다. 일시적인
  오류로 등록된 종목을 되살리려면 `/admin/` 에서 직접 삭제해야 합니다.
