...
| `DailyPnlRecord` | 당일 누적 실현손익과 마지막 기준 총자산. KST 00시 초기화 |
...
**매도 — 시장 상태와 무관하게 고정 비율**

| 조건 | 동작 |
|---|---|
| 매수가 대비 **+2%** | 익절 |
| 매수가 대비 **-2%** | 손절 |
| 이번 실행의 실현손익이 **시작 시점 총자산 대비 -10%** | 전량 청산 후 정지 |

상승장·하락장·보합장 구분 없이, 고변동성 여부와도 무관하게 같은 기준을
적용합니다. 트레일링 스탑과 시간 기반 매도는 사용하지 않습니다.

**시장 강도 판정** (대시보드 표시용 — 매매 판단에는 쓰이지 않습니다)
아래 3개 지표 중 2개 이상이 같은 방향이면 그 방향, 아니면 보합장
...
**손실 한도의 기준**은 자동매매를 시작한 시점의 총자산(KRW 잔고 + 보유 코인
평가액)입니다. 매수금액과는 무관합니다. 한도에 닿으면 보유 전량을 청산하고
프로그램이 정지하며, **정지 후 다시 시작할 수 있습니다.** 다시 시작하면 그
시점 총자산으로 한도를 새로 잡고, 한도 판정도 0 에서 다시 셉니다.

당일 누적 실현손익은 대시보드에 계속 표시되며 **KST(한국시간) 00시 기준**으로
초기화됩니다.

> 재시작할 때마다 그 시점 총자산의 -10% 만큼 손실 여유가 새로 생깁니다.
> 연속으로 다시 시작하면 하루 손실이 -10% 를 넘을 수 있습니다.
...
| `BUY_SETTLE_GRACE_SECONDS` | `30` | 매수 직후 수동매도 판정 유예 |
| `FAILED_MARKET_BLOCK_SECONDS` | `3600` | 주문 실패 종목 차단 시간 |
| `TAKE_PROFIT_RATE` | `0.02` | 매수가 대비 +2% 익절 |
| `STOP_LOSS_RATE` | `-0.02` | 매수가 대비 -2% 손절 |
| `LOSS_CUT_RATE` | `-0.10` | 손실 한도 (시작 시점 총자산 대비) |
...
- 당일 누적 손익은 `DailyPnlRecord` 에 기록되어 **서버를 재시작해도 이어집니다.**
  손실 한도로 정지한 뒤에는 시작 버튼으로 다시 시작할 수 있으며, 그때마다
  기준 총자산과 한도를 새로 잡습니다.
- 총자산을 조회하지 못하면 한도를 정할 수 없으므로 **시작이 거부됩니다**
  (`503`). 한도 없이 매매가 시작되는 것을 막기 위한 동작입니다.
